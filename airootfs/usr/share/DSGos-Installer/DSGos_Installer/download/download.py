    ...
    def start(self, metalinks=None):
        ...
        if self.download_module == "aria2":
            ...
            if self.download_module != "requests":
                logging.debug("Unknown module '%s', DSGos_Installer will use the 'requests' one as default", self.download_module)
        ...
        if not download.start(self.metalinks):
            ...
            txt = _("Can't download needed packages. DSGos_Installer can't continue.")
            raise misc.InstallError(txt)

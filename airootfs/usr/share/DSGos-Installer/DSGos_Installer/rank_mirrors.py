...
class AutoRankmirrorsProcess(multiprocessing.Process):
    ...
    def __init__(self, settings):
        ...
        self.json_obj = None
        self.DSGos_mirrorlist = "/etc/pacman.d/DSGos-mirrorlist"
    ...
    def update_mirrorlist(self):
        """ Make sure we have the latest DSGos-mirrorlist files """
        with misc.raised_privileges():
            try:
                cmd = ['pacman', '-Syy', '--noconfirm', '--noprogressbar', '--quiet', 'DSGos-mirrorlist']
                with open(os.devnull, 'w') as fnull:
                    subprocess.call(cmd, stdout=fnull, stderr=subprocess.STDOUT)
                # Use the new downloaded mirrorlist (.pacnew) files (if any)
                pacnew_path = self.DSGos_mirrorlist + ".pacnew"
                if os.path.exists(pacnew_path):
                    shutil.copy(pacnew_path, self.DSGos_mirrorlist)
            except subprocess.CalledProcessError as why:
                logging.debug('Update of DSGos-mirrorlist package failed with error: %s', why)
    ...
    def uncomment_DSGos_mirrors(self):
        """ Uncomment DSGos mirrors and comment out auto selection so
        rankmirrors can find the best mirror. """

        autoselect = "http://foss.aueb.gr/mirrors/linux/archlinux/$repo/os/$arch"

        if os.path.exists(self.DSGos_mirrorlist):
            with open(self.DSGos_mirrorlist) as mirrors:
            ...
            with misc.raised_privileges():
                # Write new one
                with open(self.DSGos_mirrorlist, 'w') as mirrors:
                    mirrors.write("\n".join(lines) + "\n")
            self.sync()
    ...
    def run_rankmirrors(self):
        if os.path.exists("/usr/bin/rankmirrors"):
            # Uncomment DSGos mirrors and comment out auto selection so
            # rankmirrors can find the best mirror.
            self.uncomment_DSGos_mirrors()

            with misc.raised_privileges():
                try:
                    # Store rankmirrors output in a temporary file
                    with tempfile.TemporaryFile(mode='w+t') as temp_file:
                        cmd = ['rankmirrors', '-n', '0', '-r', 'DSGos', self.DSGos_mirrorlist]
                        subprocess.call(cmd, stdout=temp_file)
                        temp_file.seek(0)
                        # Copy new mirrorlist to the old one
                        with open(self.DSGos_mirrorlist, 'w') as DSGos_mirrorlist_file:
                            DSGos_mirrorlist_file.write(temp_file.read())
                except subprocess.CalledProcessError as why:
                    logging.debug('Error running rankmirrors on DSGos mirrorlist: %s', why)
            self.sync()

    def filter_and_sort_arch_mirrorlist(self):
        output = '# Arch Linux mirrorlist generated by DSGos_Installer #\n'
    ...
    def run(self):
        ...
        logging.debug("Updating both mirrorlists (Arch and DSGos)...")
        self.update_mirrorlist()

        logging.debug("Filtering and sorting Arch mirrors...")
        self.filter_and_sort_arch_mirrorlist()

        logging.debug("Running rankmirrors command to sort DSGos mirrors...")

...
import json
import os
from info import DSGos_Installer_VERSION
...
class ContextFilter(Singleton):
    ...
    def get_and_save_install_id(self, is_location_screen=False):
        ...
        headers = {'X-DSGos_Installer-Installer': DSGos_Installer_VERSION}
        try:
    ...
    def get_url_for_id_request(self):
        build_server = None
        if self.api_key:
            build_srv = ['http://build', 'DSGos', 'com']
            build_srv_query = ['/hook', 'DSGos_Installer=']
    ...
    def bugsnag_before_notify_callback(self, notification=None):
        if notification is not None:
            ...
            notification.user = {"id": self.id, "name": "DSGos User", "install_id": self.install}
            return notification
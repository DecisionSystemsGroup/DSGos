...
""" Configures DSGos to encrypt user's home with encFS """
...
def backup_conf_files(dest_dir):
    ...
    for conf_file in conf_files:
        ...
        if os.path.exists(path):
            shutil.copy(path, path + ".DSGos_Installer")
    os.system("sync")
...
def setup_conf_files(dest_dir):
    ...
    with open(path, 'w') as pam_encfs:
        pam_encfs.write("# File created by DSGos_Installer (DSGos Installer)\n\n")
        ...
        pam_encfs.write("# Added by DSGos_Installer - DSGos Installer\n")

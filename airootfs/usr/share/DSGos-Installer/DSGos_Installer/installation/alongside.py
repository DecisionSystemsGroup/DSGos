...
# Leave at least 6.5GB for DSGos when shrinking
MIN_ROOT_SIZE = 6500
...
class InstallationAlongside(GtkBaseBox):
    ...
    def get_new_device(device_to_shrink):
        """ Get new device where DSGos_Installer will install DSGos
    ...
    def set_resize_widget(self, device_to_shrink):
        ...
        if max_size < 0:
            # Full DSGos does not fit but maybe base fits... ask user.
            txt = _("DSGos_Installer recommends at least 6.5GB free to install DSGos.") + "\n\n"
            ...
            txt += _("You can still install DSGos, but be carefull on which DE you choose as it might not fit in.") + "\n\n"
        ...
        self.resize_widget.set_part_title('new', 'New DSGos', new_device)
        icon_file = self.get_distributor_icon_file('DSGos')
    ...
    def get_distributor_icon_file(self, os_name):
        ...
        # No numix icon for DSGos, use our own.
        if "DSGos" in os_name:
            icons_path = os.path.join(self.settings.get('data'), "icons/48x48")
            icon_file = os.path.join(icons_path, "distributor-logo-DSGos.png")
            return icon_file
    ...
    def start_installation(self):
        ...
        if self.settings.get('bootloader_install'):
            ...
            msg = "DSGos will install the bootloader {0} in device {1}"
        ...
        if not self.testing:
            ...
            logging.info("Testing mode. DSGos_Installer will not change anything!")
        '''

...
class Bootloader(object):
    ...
    def install(self):
        ...
        if bootloader == "grub2":
            ...
            logging.debug("DSGos_Installer will install the Systemd-boot loader")
    ...
    def install_grub(self):
        ...
        if os.path.exists('/sys/firmware/efi'):
            logging.debug("DSGos_Installer will install the Grub2 (efi) loader")
            self.install_grub2_efi()
        else:
            logging.debug("DSGos_Installer will install the Grub2 (bios) loader")
    ...
    def check_root_uuid_in_grub(self):
        ...
        pattern = re.compile("menuentry 'DSGos Linux'[\s\S]*initramfs-linux.img\n}")
        ...
        if not self.settings.get('use_luks') and ruuid_str not in parse:
            ...
            if entry:
                logging.debug("Wrong uuid in grub.cfg, DSGos_Installer will try to fix it.")
    ...
    def modify_grub_default(self):
        ...
        self.set_grub_option("GRUB_THEME", "/boot/grub/themes/DSGos-Default/theme.txt")
        self.set_grub_option("GRUB_CMDLINE_LINUX_DEFAULT", cmd_linux_default)
        self.set_grub_option("GRUB_DISTRIBUTOR", "DSGos")
    ...
    def prepare_grub_d(self):
        """ Copies 10_DSGos script into /etc/grub.d/ """
        grub_d_dir = os.path.join(self.dest_dir, "etc/grub.d")
        script_dir = os.path.join(self.settings.get("DSGos_Installer"), "scripts")
        script = "10_DSGos"
    ...
    def install_grub2_efi(self):
        ...
        bootloader_id = 'DSGos_grub' if not os.path.exists('/install/boot/efi/EFI/DSGos_grub') else \
            'DSGos_grub_{0}'.format(self.random_generator())
        ...
        grub_path = os.path.join(self.dest_dir, "boot/efi/EFI/DSGos_grub", "grub{0}.efi".format(spec_uefi_arch))
    ...
    def install_systemd_boot(self):
        ...
        if not self.settings.get('use_luks'):
            conf['default'] = []
            conf['default'].append("title\tDSGos\n")
            ...
            conf['fallback'] = []
            conf['fallback'].append("title\tDSGos (fallback)\n")
            ...
            if self.settings.get('feature_lts'):
                conf['lts'] = []
                conf['lts'].append("title\tDSGos LTS\n")
                ...
                conf['lts_fallback'] = []
                conf['lts_fallback'].append("title\tDSGos LTS (fallback)\n\n")
        ...
        entry_path = os.path.join(entries_dir, "DSGos.conf")
        with open(entry_path, 'w') as entry_file:
            for line in conf['default']:
                entry_file.write(line)

        entry_path = os.path.join(entries_dir, "DSGos-fallback.conf")
        with open(entry_path, 'w') as entry_file:
            for line in conf['fallback']:
                entry_file.write(line)

        if self.settings.get('feature_lts'):
            entry_path = os.path.join(entries_dir, "DSGos-lts.conf")
            with open(entry_path, 'w') as entry_file:
                for line in conf['lts']:
                    entry_file.write(line)

            entry_path = os.path.join(entries_dir, "DSGos-lts-fallback.conf")
            with open(entry_path, 'w') as entry_file:

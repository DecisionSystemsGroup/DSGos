...
#  along with DSGos_Installer; if not, write to the Free Software
...
class Installation(object):
    ...
    def __init__(self, settings, callback_queue, packages, metalinks,
        ...
        # DSGos_Installer will store here info (packages needed, post install actions, ...)
    ...
    def start(self):
        ...
        '''
        From this point, on a warning situation, DSGos_Installer should try to continue,
    ...
    @staticmethod
    def copy_log():
        # Copy DSGos_Installer log to new installation
        datetime = "{0}-{1}".format(time.strftime("%Y%m%d"), time.strftime("%H%M%S"))
        dst = os.path.join(DEST_DIR, "var/log/DSGos_Installer-{0}.log".format(datetime))
        pidst = os.path.join(DEST_DIR, "var/log/postinstall-{0}.log".format(datetime))
        try:
            shutil.copy("/tmp/DSGos_Installer.log", dst)
            shutil.copy("/tmp/postinstall.log", pidst)
        except FileNotFoundError:
            logging.warning("Can't copy DSGos_Installer log to %s", dst)
    ...
    def download_packages(self):
        ...
        # that DSGos_Installer was able to create it before partitioning/formatting
    ...
    def prepare_pacman_keyring():
        ...
        try:
            ...
            subprocess.check_call(cmd)
            cmd = ["pacman-key", "--populate", "--gpgdir", dest_path, "archlinux", "DSGos"]
            subprocess.check_call(cmd)
        except subprocess.CalledProcessError as process_error:
    ...
    def install_packages(self):
        ...
        if not result:
            txt = _("Can't install necessary packages. DSGos_Installer can't continue.")
        ...
        for mount_point in self.mount_devices:
            ...
            if "/home" in mount_point and self.method == "automatic" and use_luks and not use_lvm:
                ...
                with open(crypttab_path, 'a') as crypttab_file:
                    line = "cryptDSGosHome /dev/disk/by-uuid/{0} {1} luks\n".format(uuid, home_keyfile)
                    crypttab_file.write(line)
                    logging.debug("Added %s to crypttab", line)
                os.chmod(crypttab_path, 0o600)

                # Add line to fstab
                txt = "/dev/mapper/cryptDSGosHome {0} {1} defaults 0 0".format(mount_point, myfmt)
                all_lines.append(txt)
                logging.debug("Added %s to fstab", txt)
                continue
    ...
    def set_scheduler(self):
        rule_src = os.path.join(self.settings.get('DSGos_Installer'), 'scripts/60-schedulers.rules')
            ...
            if os.uname()[-1] == "x86_64":
                for i in range(0, len(paclines)):
                    if paclines[i] == "#[multilib]\n":
                        ...
                        break
            paclines.append("\n")
            paclines.append("[DSGos]\n")
            paclines.append("SigLevel = PackageRequired\n")
            paclines.append("Include = /etc/pacman.d/DSGos-mirrorlist\n")
    ...
    def set_fluidsynth():
        ...
        with open(fluid_path, "w") as fluid_conf:
            fluid_conf.write('# Created by DSGos_Installer, DSGos installer\n')
    ...
    def configure_system(self):
        ...
        if self.desktop == "base":
            ...
            # See: https://github.com/DSGos/DSGos_Installer/issues/332#issuecomment-108745026
        ...
        # Add DSGos repo to /etc/pacman.conf
        ...
        logging.debug("Running DSGos_Installer post-install script")
        # Call post-install script to fine tune our setup
        script_path_postinstall = os.path.join(
            self.settings.get('DSGos_Installer'),
            "scripts",
            POSTINSTALL_SCRIPT)
        ...
        if self.settings.get('bootloader_install'):
            try:
                ...
                logging.warning("While installing boot loader DSGos_Installer encountered this error: %s", general_error)

        ...
        if response == Gtk.ResponseType.OK:
            ...
            if new_mount in self.diskdic['mounts'] and new_mount != row[COL_MOUNT_POINT]:
                ...
                if len(new_label) > 0 and not new_label.isalpha():
                    logging.debug("'%s' is not a valid label. Therefore, DSGos_Installer will set no label at all", new_label)
                    new_label = ""
    ...
    def on_partition_list_new_activate(self, button):
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if len(mylabel) > 0 and not mylabel.isalpha():
                logging.warning("%s is not a valid label. DSGos_Installer will set no label at all", mylabel)
                mylabel = ""
        ...
        if response == Gtk.ResponseType.OK:
            ...
            if line:
                ...
                if ptype == 'gpt' and not os.path.exists('/sys/firmware/efi'):
                    # Show warning (see https://github.com/DSGos/DSGos_Installer/issues/63)
                    msg = _(
                        ...
                        'DSGos_Installer will create this BIOS Boot Partition for you.')
                    show.warning(self.get_toplevel(), msg)
        ...
        if self.lv_partitions:
            for partition_path in self.lv_partitions:
                ...
                if uid in self.stage_opts:
                    ...
                    # https://github.com/DSGos/DSGos_Installer/issues/8
                    if mnt == "/":
                        fmt = True
                    if is_new:
                        if lbl != "":

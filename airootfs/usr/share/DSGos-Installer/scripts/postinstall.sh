...
set_xorg() {
	cp /usr/share/DSGos_Installer/scripts/postinstall/50-synaptics.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/50-synaptics.conf
	cp /usr/share/DSGos_Installer/scripts/postinstall/99-killX.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/99-killX.conf

	# Fix sensitivity for chromebooks
	if lsmod | grep -q cyapa; then
		cp /usr/share/DSGos_Installer/scripts/postinstall/50-cros-touchpad.conf ${CN_DESTDIR}/etc/X11/xorg.conf.d/50-cros-touchpad.conf
	fi
}

set_gsettings() {
	cp /usr/share/DSGos_Installer/scripts/set-settings "${CN_DESTDIR}/usr/bin/set-settings"
...
gnome_settings() {
	...
	if [[ "${CN_KEYBOARD_VARIANT}" != '' ]]; then
		sed -i "s/'us'/'${CN_KEYBOARD_LAYOUT}+${CN_KEYBOARD_VARIANT}'/" /usr/share/DSGos_Installer/scripts/set-settings
	else
		sed -i "s/'us'/'${CN_KEYBOARD_LAYOUT}'/" /usr/share/DSGos_Installer/scripts/set-settings
	fi
	...
	# Set gdm shell logo
	cp /usr/share/DSGos/logo.png ${CN_DESTDIR}/usr/share/DSGos/
	#systemd-nspawn -D "${CN_DESTDIR}" -u gdm -M CN_GDM gsettings set org.gnome.login-screen logo "/usr/share/DSGos/logo.png" &
	#sleep 5;
	...
	# xscreensaver config
	cp /usr/share/DSGos_Installer/scripts/postinstall/xscreensaver ${CN_DESTDIR}/home/${CN_USER_NAME}/.xscreensaver
...
cinnamon_settings() {
	...
	if [[ firefox = "${CN_BROWSER}" ]]; then
		sed -i 's|chromium|firefox|g' /usr/share/DSGos_Installer/scripts/postinstall/panel-launchers@cinnamon.org.json
	fi
	mkdir -p ${CN_DESTDIR}/home/${CN_USER_NAME}/.cinnamon/configs/panel-launchers@cinnamon.org/
	cp -f /usr/share/DSGos_Installer/scripts/postinstall/panel-launchers@cinnamon.org.json ${CN_DESTDIR}/home/${CN_USER_NAME}/.cinnamon/configs/panel-launchers@cinnamon.org/
	...
	chroot ${CN_DESTDIR} "ln -s /usr/share/DSGos/wallpapers/ /home/${CN_USER_NAME}/.cinnamon/backgrounds/DSGos" ${CN_USER_NAME}
}
...
openbox_settings() {
	# Setup user defaults
	chroot ${CN_DESTDIR} /usr/share/DSGos-openbox-setup/install.sh ${CN_USER_NAME}
...
lxqt_settings() {
	...
	echo "screens\1\desktops\1\wallpaper=/usr/share/pixmaps/wallpaper/DSGos.jpg" >> ${CN_DESTDIR}/home/${CN_USER_NAME}/.config/razor/desktop.conf
...
kde4_settings() {
	...
	# Setup user defaults
	chroot ${CN_DESTDIR} /usr/share/DSGos-kde-setup/install.sh ${CN_USER_NAME}

	## # Get zip file from github, unzip it and copy all setup files in their right places.
	## wget -q -O /tmp/master.tar.xz "https://github.com/DSGos/kde-setup/raw/master/kde-setup-2014-25-05.tar.xz"
...
mate_settings() {
	...
	if [[ $CN_BROWSER = "firefox" ]]; then
		sed -i 's|chromium|firefox|g' /usr/share/DSGos_Installer/scripts/postinstall/applications.list
	fi
	cp /usr/share/DSGos_Installer/scripts/postinstall/applications.list ${CN_DESTDIR}/usr/lib/linuxmint/mintMenu/applications.list

	# Copy panel layout
	cp /usr/share/DSGos_Installer/scripts/DSGos.layout ${CN_DESTDIR}/usr/share/mate-panel/layouts/DSGos.layout
}
...
enlightenment_settings() {
	...
	# copy DSGos menu icon
	# Setup user defaults
	chroot ${CN_DESTDIR} /usr/share/DSGos-enlightenment-setup/install.sh ${CN_USER_NAME}
...
postinstall() {
	...
	do
		echo "# >>>>ADDED BY DSGos_Installer INSTALLER<<<< #"

...
#  DSGos.py
#
#  Copyright © 2013-2015 lastwarrior
#
...
""" Main DSGos (lastwarrior Installer) module """
...
class DSGosApp(Gtk.Application):
    """ Main DSGos App class """

    def __init__(self):
        """ Constructor. Call base class """
        Gtk.Application.__init__(self,
                                 application_id="com.lastwarrior.DSGos",
    ...
    def do_activate(self):
        ...
        try:
            ...
            msg = "Cannot create DSGos main window: {0}".format(err)
            logging.error(msg)
            sys.exit(1)
        ...
        with open(self.TMP_RUNNING, "w") as tmp_file:
            tmp_file.write("DSGos {0}\n{1}\n".format(info.DSGos_VERSION, os.getpid()))
    ...
    def already_running(self):
        ...
        if os.path.exists(self.TMP_RUNNING):
            ...
            if misc.check_pid(pid):
                logging.info("DSGos with pid '%d' already running.", pid)
                return True
            else:
                # DSGos with pid 'pid' is no longer running, we can safely
...
def setup_logging():
    ...
    # File logger
    try:
        file_handler = logging.FileHandler('/tmp/DSGos.log', mode='w')
        file_handler.setLevel(log_level)
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)
    except PermissionError as permission_error:
        print("Can't open /tmp/DSGos.log : ", permission_error)
        ...
        if log_server == 'bugsnag':
            if not BUGSNAG_ERROR:
                ...
                if bugsnag_api is not None:
                    bugsnag.configure(
                        api_key=bugsnag_api,
                        app_version=info.DSGos_VERSION,
                        project_root='/usr/share/DSGos/DSGos',
                        release_stage=info.DSGos_RELEASE_STAGE)
                    ...
                    logging.info("Sending DSGos log messages to bugsnag server (using python-bugsnag).")
                else:
            ...
            logging.info("Sending DSGos logs to {0} with id '{1}'".format(log_server, myuid))
...
def check_gtk_version():
    ...
    # DSGos will be called from our liveCD that already
    # has the latest GTK version. This is here just to
    # help testing DSGos in our environment.
...
def parse_options():
    ...
    desc = _("DSGos v{0} - lastwarrior Installer").format(info.DSGos_VERSION)
    ...
    parser.add_argument(
        "-d", "--debug",
        help=_("Sets DSGos log level to 'debug'"),
        action="store_true")
    parser.add_argument(
        "-e", "--environment",
    ...
    parser.add_argument(
        "-f", "--force",
        help=_("Runs DSGos even if it detects that another instance is running"),
        action="store_true")
    parser.add_argument(
        "-i", "--disable-tryit",
    ...
    parser.add_argument(
        "-s", "--log-server",
        help=_("Choose to which log server send DSGos logs."
    ...
    parser.add_argument(
        "-u", "--update",
        help=_("Upgrade/downgrade DSGos to the web version"),
        action="store_true")
    parser.add_argument(
        "--disable-update",
        help=_("Do not search for new DSGos versions online"),
        action="store_true")
    parser.add_argument(
        "--disable-rank-mirrors",
        help=_("Do not try to rank Arch and lastwarrior mirrors"),
        action="store_true")
    parser.add_argument(
        "-v", "--verbose",
...
def update_DSGos():
    """ Runs updater function to update DSGos to the latest version if necessary """
    upd = updater.Updater(
        force_update=cmd_line.update,
        local_DSGos_version=info.DSGos_VERSION)
    ...
    if upd.update():
        ...
        # Run another instance of DSGos (which will be the new version)
...
def check_for_files():
    """ Check for some necessary files. DSGos can't run without them """
    paths = [
        "/usr/share/DSGos",
        "/usr/share/DSGos/ui",
        "/usr/share/DSGos/data"]

    for path in paths:
        if not os.path.exists(path):
            print(_("DSGos files not found. Please, install DSGos using pacman"))
            return False
...
def init_DSGos():
    """ This function initialises DSGos """

    # Sets SIGTERM handler, so DSGos can clean up before exiting
    ...
    # Check DSGos is correctly installed

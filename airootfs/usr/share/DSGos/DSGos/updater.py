...
_update_info_url = "https://raw.githubusercontent.com/KGkotzamanidis/DSGos-Installer/master/utils/update.info"
_master_zip_url = "https://github.com/KGkotzamanidis/DSGos-Installer/archive/master.zip"
_update_info = "/usr/share/DSGos/update.info"
...
class Updater(object):
    def __init__(self, local_DSGos_version, force_update=False):
        self.remote_version = ""
        self.local_DSGos_version = local_DSGos_version
        self.md5s = {}
        ...
        #     logging.warning("Cannot not find %s file. DSGos will not be able to update itself.", _update_info)
        #     return
    ...
    def is_remote_version_newer(self):
        """ Returns true if the Internet version of DSGos is newer than the local one """

        if len(self.remote_version) < 1:
            return False

        # Version is always: x.y.z
        local_ver = self.local_DSGos_version.split(".")
    ...
    def update(self):
        ...
        if update_DSGos:
            logging.debug("Downloading DSGos %s...", self.remote_version)
            zip_path = "/tmp/DSGos-{0}.zip".format(self.remote_version)
    ...
    def download_master_zip(zip_path):
        """ Download new DSGos version from github """
    ...
    def unzip_and_copy(self, zip_path):
        """ Unzip (decompress) a zip file using zipfile standard module
            and copy DSGos's files to their destinations """
        import zipfile
        ...
        with zipfile.ZipFile(zip_path) as zip_file:
            ...
            for member in zip_file.infolist():
                ...
                dst_full_path = os.path.join(
                    "/usr/share/DSGos",
                    full_path.split("/tmp/DSGos-master/")[1])
                if os.path.isfile(dst_full_path):
                    if dst_full_path in self.md5s:
                        if "update.info" not in dst_full_path and self.md5s[dst_full_path] != get_md5_from_file(full_path):
                            logging.warning(
                                _("Wrong md5 (%s). Bad download or wrong file, DSGos won't update itself"),
                                member.filename)
                            all_md5_ok = False
                            break
                    else:

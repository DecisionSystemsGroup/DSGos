...
On a warning situation, DSGos should try to continue, so we need to catch the exception here.
...
def setup_luks(luks_device, luks_name, luks_pass=None, luks_key=None):
    ...
    logging.debug("DSGos will setup LUKS on device %s", luks_device)
...
class AutoPartition(object):
    ...
    def run(self):
        ...
        if self.lvm:
            logging.debug("DSGos will setup LVM on device %s", devices['lvm'])
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename="/tmp/DSGos-autopartition.log", level=logging.DEBUG)

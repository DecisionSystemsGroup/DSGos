...
class Bootloader(object):
    ...
    def install(self):
        ...
        if bootloader == "grub2":
            ...
            logging.debug("DSGos will install the Systemd-boot loader")
    ...
    def install_grub(self):
        ...
        if os.path.exists('/sys/firmware/efi'):
            logging.debug("DSGos will install the Grub2 (efi) loader")
            self.install_grub2_efi()
        else:
            logging.debug("DSGos will install the Grub2 (bios) loader")
    ...
    def check_root_uuid_in_grub(self):
        ...
        if not self.settings.get('use_luks') and ruuid_str not in parse:
            ...
            if entry:
                logging.debug("Wrong uuid in grub.cfg, DSGos will try to fix it.")
    ...
    def prepare_grub_d(self):
        ...
        script_dir = os.path.join(self.settings.get("DSGos"), "scripts")
        script = "10_antergos"

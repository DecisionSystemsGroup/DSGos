...
#  along with DSGos; if not, write to the Free Software
...
class Installation(object):
    ...
    def __init__(self, settings, callback_queue, packages, metalinks,
        ...
        # DSGos will store here info (packages needed, post install actions, ...)
    ...
    def start(self):
        ...
        '''
        From this point, on a warning situation, DSGos should try to continue,
    ...
    @staticmethod
    def copy_log():
        # Copy DSGos log to new installation
        datetime = "{0}-{1}".format(time.strftime("%Y%m%d"), time.strftime("%H%M%S"))
        dst = os.path.join(DEST_DIR, "var/log/DSGos-{0}.log".format(datetime))
        pidst = os.path.join(DEST_DIR, "var/log/postinstall-{0}.log".format(datetime))
        try:
            shutil.copy("/tmp/DSGos.log", dst)
            shutil.copy("/tmp/postinstall.log", pidst)
        except FileNotFoundError:
            logging.warning("Can't copy DSGos log to %s", dst)
    ...
    def download_packages(self):
        ...
        # that DSGos was able to create it before partitioning/formatting
    ...
    def install_packages(self):
        ...
        if not result:
            txt = _("Can't install necessary packages. DSGos can't continue.")
    ...
    def set_scheduler(self):
        rule_src = os.path.join(self.settings.get('DSGos'), 'scripts/60-schedulers.rules')
    ...
    def set_fluidsynth():
        ...
        with open(fluid_path, "w") as fluid_conf:
            fluid_conf.write('# Created by DSGos, Antergos installer\n')
    ...
    def configure_system(self):
        ...
        if self.desktop == "base":
            ...
            # See: https://github.com/Antergos/DSGos/issues/332#issuecomment-108745026
        ...
        if self.settings.get('bootloader_install'):
            try:
                ...
                logging.warning("While installing boot loader DSGos encountered this error: %s", general_error)

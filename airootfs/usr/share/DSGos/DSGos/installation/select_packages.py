...
class SelectPackages(object):
    ...
    def select_packages(self):
        ...
        if len(self.alternate_package_list) > 0:
            ...
            try:
                url = 'http://install.antergos.com/packages-{0}.xml'.format(info.DSGos_VERSION.rsplit('.')[-2])
                logging.debug("Getting url %s...", url)
                req = urllib.request.Request(url, headers={'User-Agent': 'Mozilla/5.0'})
                packages_xml = urllib.request.urlopen(req, timeout=10)
            except urllib.error.URLError as url_error:
                # If the installer can't retrieve the remote file DSGos will use
                # a local copy, which might be updated or not.
                if "production" == info.DSGos_RELEASE_STAGE:

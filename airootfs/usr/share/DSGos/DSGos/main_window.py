...
class MainWindow(Gtk.ApplicationWindow):
    ...
    def __init__(self, app, cmd_line):
        Gtk.ApplicationWindow.__init__(self, title="DSGos", application=app)

        self.MAIN_WINDOW_WIDTH = 875
        self.MAIN_WINDOW_HEIGHT = 550

        logging.info("DSGos installer version %s", info.DSGos_VERSION)

        self.settings = config.Settings()
        self.ui_dir = self.settings.get('ui')

        if not os.path.exists(self.ui_dir):
            DSGos_dir = os.path.join(os.path.dirname(__file__), './')
            self.settings.set('DSGos', DSGos_dir)
        ...
        if cmd_line.cache:
            logging.debug("DSGos will use '%s' as a source directory for cached xz packages", cmd_line.cache)
        ...
        if cmd_line.environment:
            ...
            if desktop in desktop_info.DESKTOPS:
                ...
                logging.debug("DSGos will install the %s desktop environment", desktop)

        self.ui = Gtk.Builder()
        path = os.path.join(self.ui_dir, "DSGos.ui")
        ...
        title = "DSGos {0}".format(info.DSGos_VERSION)

...
class Download(object):
    """ Class to download packages using urllib
        ...
        DSGos installation using urllib """
    ...
    def start(self, downloads):
        ...
        while len(downloads) > 0:
            ...
            if os.path.exists(dst_path):
                ...
                logging.debug("File %s already exists, DSGos will not overwrite it", element['filename'])
                ...
                try:
                    ...
                    logging.debug("Error copying %s to %s, so DSGos will try to download it", dst_cache_path, dst_path)

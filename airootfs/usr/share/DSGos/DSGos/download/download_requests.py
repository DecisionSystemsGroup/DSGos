...
class Download(object):
        ...
        while len(downloads) > 0:
            ...
            if os.path.exists(dst_path):
                ...
                if element['hash'] is not None and element['hash'] != md5:
                    logging.debug(
                        "MD5 hash (url:%s, file:%s) of file %s do not match! DSGos will download it",
                        element['hash'],
                        md5,
                        element['filename'])
                    ...
                    logging.debug(
                        "File %s already exists, DSGos will not overwrite it",
                        element['filename'])
                ...
                if element['hash'] is not None and element['hash'] != md5:
                    ...
                    try:
                        ...
                        logging.debug(
                            "Error copying %s to %s, so DSGos will download it",
                            dst_cache_path,
                            dst_path)
                    ...
                    try:
                        ...
                        if r.status_code == requests.codes.ok:
                            ...
                            with open(dst_path, 'wb') as xz_file:
                                ...
                                if element['hash'] is not None and element['hash'] != md5:
                                    logging.debug(
                                        "MD5 hash (url:%s, file:%s) of file %s do not match! DSGos will try another mirror.",
                                        element['hash'],
                                        md5,
                                        element['filename'])
                            ...
                            download_error = True
                            msg = "Can't download {0}, DSGos will try another mirror.".format(url)
                            logging.debug(msg)
                            continue
                    except (socket.timeout, requests.exceptions.Timeout) as connection_error:
                        download_error = True
                        msg = "Can't download {0} ({1}), DSGos will try another mirror.".format(url, connection_error)
                        logging.debug(msg)
                        continue
                    except requests.exceptions.ConnectionError as connection_error:
                        download_error = True
                        msg = "Can't download {0} ({1}), DSGos will try another mirror in a minute.".format(url, connection_error)
                        logging.debug(msg)
    ...
    def queue_event(self, event_type, event_text=""):
        """ Adds an event to DSGos event queue """
